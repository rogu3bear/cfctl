...
async fn parse_success_response(
    ...
    match contract.body_mode {
        ...
        ResponseBodyModeV1::R2PrivateObjectDigest => {
            Err(CloudflareError::UnsupportedResponseContract(
                "R2 private object digests require the dedicated streaming executor".to_owned(),
            ))
        }
...
fn exact_private_object_etag(headers: &HeaderMap) -> Result<String> {
    let values = headers
    ...
    if values.len() != 1 {
        return Err(CloudflareError::InvalidRequestBody(
            "R2 private object digest requires exactly one ETag".to_owned(),
        ));
    }
    values[0]
        .to_str()
        .ok()
        .filter(|etag| !etag.is_empty() && etag.len() <= 1024 && etag.trim() == *etag)
        .map(str::to_owned)
        .ok_or_else(|| {
            CloudflareError::InvalidRequestBody(
                "R2 private object digest received an invalid ETag".to_owned(),
            )
        })
}
...
#[cfg(test)]
use d1_import::{
    D1ImportIngestOutcome, D1ImportInitRejection, accepted_d1_import_ingest_response,
    accepted_d1_import_init_response, accepted_d1_import_poll_outcome,
    classify_d1_import_ingest_response, validate_d1_import_poll_response,
    validate_d1_import_upload_url,
};
use d1_import::{
    D1ImportPollOutcome, bounded_d1_import_upload, classify_d1_import_init_response,
    classify_d1_import_poll_response, finish_d1_import, persist_import_response,
    persist_import_uncertainty, validated_d1_import_upload_etag,
};
...
mod approved_mln_import_tests {
    ...
    #[test]
    fn approved_import_upload_url_requires_a_canonical_r2_presigned_put_target() {
    ...
    fn approved_import_classifies_ingest_and_poll_before_later_actions() {
        ...
        assert_eq!(
            ...
            super::D1ImportIngestOutcome::InProgress("owned".to_owned())
        );
        let invalid_ingest = [
