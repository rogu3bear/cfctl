...
mod real_wrangler {
    ...
    async fn prove_owned_interpreter_drift(fixture: &Fixture, plan: &PlanV1) {
        ...
        // Homebrew Node may load libnode relative to its executable. Preserve
        // that read-only library layout while keeping both executable copies
        // owned and mutable for the path/byte drift assertions below.
        let libraries = original
            .parent()
            .expect("Node bin directory")
            .join("../lib");
        if libraries.is_dir() {
            symlink(
                fs::canonicalize(libraries).expect("Node library directory"),
                bin.join("lib"),
            )
            .expect("fixture library layout");
        }
        let bin = bin.join("bin");
        fs::create_dir(&bin).expect("owned executable directory");
