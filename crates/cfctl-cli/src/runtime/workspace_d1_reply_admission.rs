...
use super::{
    prelude::{
        AuthCredential, BTreeSet, CallInput, CapabilityV1, CatalogSnapshot, CliError, OpenOptions,
...
fn validate_candidate_bytes(bytes: &[u8]) -> Result<Candidate> {
    ...
    apple_mail_inbox_source::validate_projection_binding(source_prerequisites, projection)?;
...
fn validate_source_prerequisites(prerequisites: &Map<String, Value>) -> Result<()> {
    for plane in PREREQUISITE_KEYS {
        ...
        if *plane == "apple_mail_inbox" {
            apple_mail_inbox_source::validate(receipt)?;
            continue;
        }
...
fn read_committed_compiler_support(
    ...
    let bytes =
        super::import_planning::git_authority_bytes(repository_root, &["show", &blob_spec])?;
    if bytes.is_empty() || bytes.len() as u64 > MAX_CANDIDATE_BYTES {
        return Err(CliError::Input(
