...
must report the PATH entry resolving to the running executable; that is not
proof PATH `git_commit` matches this checkout HEAD. A different or missing PATH
binary, PATH git_commit differs from this cfctl checkout HEAD, and drifted
agent instructions are unhealthy states, not warnings.
...
preserving historical graph and evidence records. Discovery skips directories
by exact name, not by category: `.build`, `.cache`, `.git`, `.swiftpm`,
`.terraform`, `.wrangler`, `Carthage`, `DerivedData`, `Pods`, `__fixtures__`,
`cargo-home`, `coverage`, `dist`, `fixtures`, `node_modules`, `target`,
`test-data`, `test_data`, `testdata`, `var`, and `vendor`. A generated
directory whose name is not on that list is still walked. `build` is walked
because it is also a legitimate source directory name. `included_entry` in
`crates/cfctl-workspace` is the list.
