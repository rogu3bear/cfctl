...
cf_setup_log_pipe "shared-aliases" "build-normalize-secondary-shared"
...
jq -r '
  ...
  "  return { local: normalized.slice(0, atIndex), domain: normalized.slice(atIndex + 1) };\n" +
  "}\n\n" +
  "export default {\n" +
  "  async email(message) {\n" +
...
echo "${ZONES_JSON}" | jq -r '.[] | @base64' | while read -r zone_row; do
  ...
  echo "${ALIASES_JSON}" | jq -r '.[]' | while read -r alias; do
    ...
    if [[ -n "${existing_rule_id}" ]]; then
      echo "Updating rule for ${address}: ${existing_rule_id}"
      update_response="$(
      ...
      if ! echo "${update_response}" | jq -e '.success == true' >/dev/null; then
        echo "Failed to update rule for ${address}" >&2
        exit 1
      fi
    else
