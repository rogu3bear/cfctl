...
| Reopening the settled Worker carrier creates two deployment authorities | The production checklist binds server-rendered Leptos to `cfctl-site` with Workers Assets; Pages would change the build/runtime model | High | High | Keep Workers canonical for this launch. Admit a Pages variant only through a separate architecture and release decision. |
