...
| 2 | Close exact `workers.dev` delivery and live readback | The first-read route, Worker SSR, and immutable assets pass locally. Historical deployment evidence proves that a `cfctl-site` service has existed, but it does not prove the current candidate or current provider state. | High activation and truthfulness | High | Medium | Re-read the service and previous version, then use separate governed upload and 100% promotion plans for the exact merged candidate. |
