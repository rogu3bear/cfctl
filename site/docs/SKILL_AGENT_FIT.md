...
| Deploy + cf-deploy | Governed plan/apply/readback through the CLI | Load; Workers Assets artifact and account mapping are proven | `workers.dev` first is selected; live reads proved the `sp5qybrsvz` subdomain and no existing `cfctl-site`, so plan/apply now waits only for a committed release tree and exact operation approval. |
