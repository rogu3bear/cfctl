...
- **Failure:** route or asset drift, callback leakage, external requests,
  ...
  execution.
- **Stop/rollback:** do not bind any custom domain after a failed preview;
  compensate or roll back the exact Worker version. Stop user testing
  immediately on any callback disclosure.
...
Commit the approved release tree, freshly read the intended account and Worker
service, then create an exact `workers.dev` deployment plan and review its
operation ID. Further Leptos refactoring has a worse learning-to-cost ratio
than closing that provider evidence boundary.
