...
- [ ] Close the observed omitted-`source` project-mode admission gap without treating omission alone as direct-upload evidence; require exact corroborating deployment identity and fail closed on Git, repository-build, or contradictory evidence before a provider process. A configured direct-upload output directory remains valid. Artifact, producer, replay, exact-ID, polling, and rectification protections remain installed; the closed failed operation cannot replay.
...
| 3 | Repair and adopt omitted-`source` project-mode admission | Exact no-write AOS failure preserved; PR #142 artifact/producer boundary remains installed | Omission is accepted only with matching canonical/latest direct-deployment identity, idle repository stages, no repository build command/root, and no nested Git evidence; a direct-upload output directory remains valid, while ambiguity creates no operation and starts no provider process | Source/test/review/integration/install only; no AOS deployment |
| 3a | Prove one fresh AOS direct upload through the installed governed path | Edge 3 merged and installed; AOS owner separately dispatches a brand-new exact operation | Artifact and producer manifest, direct-project admission, approval/run receipt, provider-returned deployment ID, exact terminal production readback, unchanged custom-domain/DNS/secret state, and no replay of `dc8c34c3…` | One exact AOS Pages deployment only; no raw Wrangler/API/dashboard bypass |
...
| AOS direct-upload project received a bodyless Pages deployment request | Codex + AOS deployment owner | 2026-08-18 | NEW PROJECT-MODE ADMISSION DEFECT; PROVIDER ACCEPTANCE BLOCKED | PR #142's deterministic artifact/producer boundary remains installed, but the next AOS attempt stopped before operation creation because the live project omitted `source`. The source successor admits that omission only when canonical/latest deployment identity, direct `ad_hoc` stages, no repository build command/root, and absence of nested Git evidence agree; a direct-upload output directory such as `target/site` remains valid, while contradictory evidence remains blocked. Exact source proof, merge/install adoption, and then one separately authorized fresh AOS operation remain distinct. Never replay `dc8c34c3…` |
