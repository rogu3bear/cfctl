...
- [ ] Close the observed omitted-`source` project-mode admission gap without treating omission alone as direct-upload evidence; require exact corroborating deployment identity and fail closed on Git or contradictory evidence before a provider process. Artifact, producer, replay, exact-ID, polling, and rectification protections remain installed; the closed failed operation cannot replay.
...
| 3 | Repair and adopt omitted-`source` project-mode admission | Exact no-write AOS failure preserved; PR #142 artifact/producer boundary remains installed | Omission is accepted only with matching canonical/latest direct-deployment identity, idle repository stages, empty build configuration, and no nested Git evidence; ambiguity creates no operation and starts no provider process | Source/test/review/integration/install only; no AOS deployment |
