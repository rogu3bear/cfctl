...
| TM-008 | Browser-local attacker or shoulder-surfer | Access to device history, DOM, or clipboard near callback use | Recover a callback value after navigation or copy | Short-lived authorization confidentiality loss | OAuth callback values | Query replacement, no-store, two-minute timeout, pagehide, visibility hidden, and pageshow only when `event.persisted` (`site/src/oauth.rs::clears_sensitive_callback_on_page_event`, `site/src/routes/oauth_callback.rs`) | Clipboard cannot be recalled; Safari/Chromium clipboard permission still varies | Keep value one-time and short-lived in CLI; prove callback lifecycle on the live origin before public OAuth | No sensitive logging; investigate only user-reported failures with redacted metadata | low | high | medium |
