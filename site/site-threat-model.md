...
- Deployment assumption: publish to `workers.dev` first, validate live headers,
  routes, and assets, then bind `cfctl.com` through an exact account-pinned
  cfctl plan. Source configuration and a local build are not deployment proof.
...
| Published Worker and asset bundle | Substitution can execute attacker code in the trusted `cfctl.com` origin or mislead operators | I, A |
